...
```python
x, w = torch.randn(1, 2, 3), torch.randn(4, 3, 4)
...
1. initialize `jacobian = torch.zeros(*x.shape, *w.shape, *x.shape)`
2. put values from `w` in `jacobian` 
3. if necessary, collapse/reshape `jacobian` 
 
with the crux being step 2. 
...
So our jacobian should be organized like this

$$
\frac{\partial j_{ijklmn}}{\partial x_{mn}}= 
\begin{cases}
w_{kl} & \text{if } \ i = m, j = n & \text{and} \ z_{ijkl} \ \text{is defined}\\
0 & \text{else}
\end{cases}
$$
...
```
We can write a function to give us the pattern for the diagonal
```python
...
```
The subscript names dont matter as long as you sum the right stuff. For 1. we could do `fucker->fker`.  We can write a simple function that generates this einsum pattern by using elipses. 
