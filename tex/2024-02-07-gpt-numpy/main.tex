...
module using \pyth{einsum}. 

The way I want to set up the autograd engine is to have 
a \pyth{engine.py} file that contains a class called \pyth{Parameter} which holds data, grad, shape,..etc, and 
...
GPT implementation. 
\par To figure out what class methods we need in \pyth{Parameter} lets start by listing the functions we want, and how 
those require \pyth{Parameter} to behave. e.g. \pyth{nn.LayerNorm} calculation uses 
mean and variance, so to implement that,
\pyth{Parameter} needs some kind of \pyth{mean} method.
\begin{enumerate}
...
\end{minted}
First we need a way to unpack a \pyth{Parameter} along an axis, and second we need masking support. All together
