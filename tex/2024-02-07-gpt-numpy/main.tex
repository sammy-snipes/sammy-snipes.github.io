...
I thought it might be fun to write chat GPT in numpy. The way were going to do this is first write it in pytorch,
then write a \emph{minimal} autograd engine in numpy. Lets start with the pytorch implementation. 

\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.8\textwidth]{figures/gpt2-arc.jpg}
  \end{center}
  \caption{GPT2 architecture}\label{fig:}
\end{figure}

Architecture is nothing crazy. For the implementation I more or less copied Andrej Kaparhty's 
\href{https://github.com/karpathy/ng-video-lecture}{here}, except I stuck the multihead attention in one
module using some \pyth{einsum}
...
\end{minted}
Now we can stare at this and figure out what we need to implement in our autograd engine. The way I want this set up 
is a \pyth{engine.py} file that contains a class called \pyth{Parameter} which holds data, grad, shape,..etc, and 
implements some dunders like \pyth{__add__}, \pyth{__mul__} while tracking gradient. Then well have a \pyth{functions.py}
file that has \pyth{Parameter} implementations for \pyth{softmax}, \pyth{gelu}, \pyth{layernorm}. Finally, there 
will be a \pyth{models.py} file that has class wrappers to make it all feel like pytorch along with the
GPT implementation. 
\par To figure out what class methods we need in \pyth{Parameter} lets start by listing the functions we want
\begin{enumerate}
  \item \pyth{nn.Embedding}. This is just a lookup table; nothing special required. 
  \item \pyth{einsum}. This is explicitly used in the mutlihead-attention. \pyth{nn.Linear} is just an einsum along the
    last two dimensions of the input, so we get that for free by implementing \pyth{einsum}.
  \item \pyth{nn.GELU}. Nothing special required. 
  \item \pyth{nn.Dropout}. Nothing special required.
  \item \pyth{einops.rearrange}. This is easy to implement and read, nothing special required. 
  \item \pyth{nn.Softmax}. Nothing special required.
  \item \pyth{nn.CrossEntropyLoss}. Let $Y$ be the target, and our prediction 
    $\hat{Y} = \text{model}(X)$, then CE loss is given by
    \begin{equation}
      CE(Y, \hat{Y}) = -\frac{1}{N}\sum_{n, c}Y \odot \log \bigg(\text{softmax} \big(\hat{Y}\big) \bigg) 
    \end{equation}
    The haddamard product and sum can be handled by one einsum pattern \pyth{ij,ij->}, and we already
    have softmax above, so thats missing is \pyth{torch.log} which we can write a function for, 
    and scalar multiplication which we can handle with the \pyth{__rmul__} dunder.  
  \item \pyth{nn.LayerNorm}. The equation is
    \begin{equation}
      ln = \frac{x - \text{E}[x]}{\sqrt{\text{Var}[x] + \epsilon}} * \gamma + \beta
    \end{equation}
    where $\gamma, \beta$ are the same shape as the dimensions being normalized. This will require a \pyth{.mean()} 
    method. We can get variance with
    \begin{equation}
      \text{var} = \text{E}[x^2] - \text{E}[x]^2
    \end{equation}
    For all this we need the following dunders: \pyth{__pow__}, \pyth{__mul__}, \pyth{__add__}, \pyth{__sub__}. 
    Instead of implemting \pyth{__div__} ill use negative exponents and multiplication so itll look like 
    \begin{minted}{python}
    ln = (x - x.mean()) * ((x**2).mean() - (x.mean())**2 + eps) ** -0.5 ...stuff...
    \end{minted}
\end{enumerate}
That covers it for functions. For class methods theres two things we need that have yet to pop up. First,
in the torch attention implementation we split the tensor in the key, query, value. So we need some way to 
split our \pyth{Parameter} along an axis and track gradients. Second, we need to handle masking.
All together the class methods we need are:
\begin{enumerate}
  \item \pyth{__add__}
  \item \pyth{__sub__}
  \item \pyth{__mul__}
  ...
  \item \pyth{mean()}
  \item \pyth{split()}
  \item \pyth{masked_fill()}
\end{enumerate}
and then some utility stuff
\begin{enumerate}[resume]
  \item \pyth{backward()}, something to trigger backpropogation 
  \item \pyth{sum()}, nice to have
  \item \pyth{broadcast_helper}, broadcasting introduces some complications to backprop. Since addition, subtraction
...
\section{The class}
The \pyth{backward()} method is going to dictate a lot of what we do here so lets settle that first. I liked the 
way Andrej Kaparthy laid it out in his micrograd \href{https://www.youtube.com/watch?v=VMj-3S1tku0}{video}. Every \pyth{Parameter} will have a lambda function 
\pyth{_backward} that dictates how its gradient interacts with its children during backpropogation, and calling
the class method \pyth{backward()} will call the \pyth{_backward} function of everything in the computational graph. 
...
class Parameter:
    ...
    def backward(self):
        ...
        for node in stack[::-1]:
            node._backward()
\end{minted}
For backward we first assert that its a scalar, and then call the \pyth{_backward} method of everything in the 
reverse order of the computational graph. 
\subsection{split}
We want to be able to split a \pyth{Parameter}, do stuff to the children, and have the gradient backpropogate 
correctly. 
\begin{minted}{python}
    def split(self, dim=0) -> List["Parameter"]:
        ...
        return kids
\end{minted}
Using \pyth{np.moveaxis} to bring the split axis to the zero dimension, we split the \pyth{Parameter} into children. 
To backpropogate we use \pyth{np.moveaxis} with the reverse operands to return a view, and then just add the child grad.
...
\begin{minted}{python}
    def masked_fill(self, mask: np.ndarray, value: float) -> "Parameter":
        ...
        return out
\end{minted}
For this, any value that gets masked has a zero gradient. For mask you would pass a boolean array like \pyth{x == 0}
\subsection{sum}
\begin{minted}{python}
    def sum(self, dim=None, keepdim=False) -> "Parameter":
        ...
        return out
\end{minted}
When \pyth{keepdim == True} dims are summed to be 1, so there are no broadcasting issues. If its false,
suppose \pyth{a.shape = [2, 3, 4]}, and \pyth{c = a.sum(-1, 1)}, then backward pass will have
\pyth{a.grad += c.grad}, which bricks since adding shapes
\pyth{[2, 3, 4] += [3]} isnt valid. The solution is expand collapsed dims to 1.
...
\end{equation}
and we use the same reshaping we used for \pyth{sum}
\subsection{dunder}
Lets first solve the broadcasting issue. Lets say somewhere in our network we have \pyth{a, b} of shapes
\pyth{[2, 3, 4],  [2, 3]}, and \pyth{c = a + b}. The backward pass will require
\begin{minted}{python}
  a.grad += c.grad
  b.grad += c.grad
\end{minted}
but line 2 fails since \pyth{[2, 3, 4]} cant be broadcast into \pyth{[2, 3]}. So what is the grad supposed to be?
When \pyth{a, b} get added, the broadcasting adds \pyth{b} to each array along the 0'th dimension of \pyth{a}. 
So the gradient w.r.t \pyth{a} is \pyth{c.grad} summed along the 0'th dimension. In general, to get the grad w.r.t. the broadcasted
operand you just sum the grad from the left until it has the same shape as the operand. 
\par Theres one more case to handle. If we had \pyth{a, b} of shapes \pyth{[2, 3, 4], [2, 1, 4]}, we'll throw a 
broadcasting error in the backward pass since \pyth{[2, 3, 4]} can be broadcast into \pyth{[2, 1, 4]}
The solution is sum \pyth{c.grad} to 1 along the 1'th dimension. \pyth{[2, 3, 4] -> [2, 1, 4]}. In general, the grad
...
\end{minted}
First we sum from the left until \pyth{grad} and \pyth{a} have the same number of dimensions, then whichever
dims have length 1 in \pyth{a} get summed to 1 in \pyth{grad}. With this out of the way we can write our dunders. 
These are all straight forward so ill show addition and multiplication. \pyth{__pow__} doesnt have 
...
\end{minted}
SciPy is for \pyth{GELU}, and the einops stuff is because I'm going to implement a rearrange function for 
\pyth{Parameter} that just calls the einops version lol. We've built out far enough that a lot of the function
...
\pyth{ij,jk->ik}, the grad w.r.t. operand 0 is \pyth{ik,jk->ij}. Problems arise if you sum to a scalar. E.g. 
\pyth{ij,jk->} would reverse to \pyth{,jk-ij}. 
\par This bricks becasue operand zero needs a string, and the \pyth{i} in the output string is unknown.
The first problem we solve by making grad 1d if its scalar, and assigning it a unique letter. 
We solve the second issue by making the out string of our reversed einsum only the known dimensions. i.e. the dimensions of the operand contained in the other operands or output. So the grad string of \pyth{ij,jk->} would be 
\pyth{q,jk->j}, where \pyth{q} is an arbitrary letter. 
...
\par At this point we are basically done, really. We have core operations in our \pyth{Parameter} class methods and 
...
\end{minted}
The \pyth{_from_torch} method allows me to initialize an \pyth{Embedding} object from a \pyth{nn.Embedding}
by copying+detaching its weight, converting it to a \pyth{Parameter}, and assigning it to the object. The 
\pyth{_weight_to_param} does what the name implies, and theres another method \pyth{_do_nothing} used for
assigning non-learnable attributes like softmax dim, layer norm shape, ...etc. Finally in theres a global function
\pyth{convert_nn_module} that converts \pyth{nn.Module}'s to their \pyth{Parameter} based equivalent by checking a 
dictionary. 
\begin{minted}{python}
import torch.nn as nn
CONVERSION_DICT = {
